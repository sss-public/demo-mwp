...
describe("ERC1967ProxyHarness::approve(address,uint256) - reverted", async () => {
  let acct0xf39f,
  ...
  it("0x440c19", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x40ae08

    erc1967proxyHarness0xcf7eMakeWorldPeaceV3 = await ethers.getContractAt("MakeWorldPeaceV3", erc1967proxyHarness0xcf7e.address);
    await expect(erc1967proxyHarness0xcf7eMakeWorldPeaceV3.approve(ethers.constants.AddressZero, 0)).to.be.revertedWith(
      "ERC20: approve to the zero address"
    );
  });

  it("0x440bb2", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x407e56
    ...
    await expect(
      acct0xf39f.sendTransaction({
        ...
        data: "",
      })
