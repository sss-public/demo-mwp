...
describe("ERC1967ProxyHarness::upgradeToV2() - reverted", async () => {
  let acct0xf39f,
  ...
  it("0x441033", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x3f3d05
    ...
    await expect(
      acct0xf39f.sendTransaction({
        ...
        data: "",
      })
    ).to.be.revertedWithoutReason(/* value transfer to nonpayable call: `upgradeToV2()` */);
  });

  it("0x440ea1", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x4275da

    erc1967proxyHarness0xcf7eMakeWorldPeaceV3 = await ethers.getContractAt("MakeWorldPeaceV3", erc1967proxyHarness0xcf7e.address);
    await expect(erc1967proxyHarness0xcf7eMakeWorldPeaceV3.upgradeToV2()).to.be.revertedWith(
      "This function can be only called once and should be the first function to be called after the upgrade progress!"
    );
  });
