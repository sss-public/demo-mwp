...
describe("ERC1967ProxyHarness::proxiableUUID() - reverted", async () => {
  let acct0xf39f,
  ...
  it("0x440f2a", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x3f5bb8
    ...
    await expect(
      acct0xf39f.sendTransaction({
        ...
        data: "",
      })
    ).to.be.revertedWithoutReason(/* value transfer to nonpayable call: `proxiableUUID()` */);
  });

  it("0x440b7d", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x41f9e3

    erc1967proxyHarness0xcf7eMakeWorldPeaceV3 = await ethers.getContractAt("MakeWorldPeaceV3", erc1967proxyHarness0xcf7e.address);
    await expect(erc1967proxyHarness0xcf7eMakeWorldPeaceV3.proxiableUUID()).to.be.revertedWith(
