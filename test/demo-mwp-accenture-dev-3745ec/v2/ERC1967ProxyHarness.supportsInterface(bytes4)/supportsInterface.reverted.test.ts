...
describe("ERC1967ProxyHarness::supportsInterface(bytes4) - reverted", async () => {
  let acct0xf39f,
  ...
  it("0x3cb21d", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x39a4e4
    ...
    await expect(
      acct0xf39f.sendTransaction({
        ...
        data: "",
      })
    ).to.be.revertedWithoutReason(/* value transfer to nonpayable call: `supportsInterface(bytes4)` */);
  });

  it.skip("0x3cb0d6", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x399fd6

    erc1967proxyHarness0x9fe4MakeWorldPeaceV2 = await ethers.getContractAt("MakeWorldPeaceV2", erc1967proxyHarness0x9fe4.address);
    await expect(erc1967proxyHarness0x9fe4MakeWorldPeaceV2.supportsInterface("0x01010101")).to.be.revertedWithoutReason();
  });
