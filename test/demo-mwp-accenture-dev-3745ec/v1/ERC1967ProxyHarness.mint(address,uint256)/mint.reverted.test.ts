...
describe("ERC1967ProxyHarness::mint(address,uint256) - reverted", async () => {
  ...
  it("0x386a5f", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x382bac
  ...
  it("0x3869d7", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x37b744

    erc1967proxyHarness0xe7f1MakeWorldPeace = await ethers.getContractAt("MakeWorldPeace", erc1967proxyHarness0xe7f1.address);
    await expect(erc1967proxyHarness0xe7f1MakeWorldPeace.mint(ethers.constants.AddressZero, 0)).to.be.revertedWith(
      "ERC20: mint to the zero address"
    );
  });

  it("0x386802", async () => {
    // http://172.16.0.71/internal/root/0x374624/path/0x37d4db

    erc1967proxyHarness0xe7f1MakeWorldPeace = await ethers.getContractAt("MakeWorldPeace", erc1967proxyHarness0xe7f1.address);
    await expect(erc1967proxyHarness0xe7f1MakeWorldPeace.mint(ethers.constants.AddressZero, 0x821e19e0c9bab2410000n)).to.be.revertedWith(
      "Exceed maximum allowed supply (10,000) within 3 minutes."
    );
  });
